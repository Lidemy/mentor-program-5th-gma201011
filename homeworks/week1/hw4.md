...
2.把你的工作檔案丟到資料夾裡面後，可以用 `touch .gitignore`  建立一個忽略清單，接著用 `vim .gitignore` 來輸入要被忽略的檔案名稱，而不需要加入版本控制的檔案，可能會是一些系統產生或是使用者個人的不重要檔案。
...
4.在工作檔案完成到一個階段時，可以用 `git commit -m` 後面加上版本名，來提交一個新的版本。

5.而已經加入過版本控制的舊有檔案，可以直接用 `git commit -am `，`-a` 會把已經修改過的檔案加入 stage 區（注意新的檔案不會），接著 commit 提交一個新版本。
...
例如：開一個新的分支「new-feat」，該分支裡面同樣會有 master 主分支裡的 A、B 兩個 commit，而這個新的 new-feat 分支可以讓使用者單獨作業外，也可以併進去原本的主分支 master 裡。
...
1.master 分支有兩個 commit A 與 B，A 版本寫 1，B 版本寫 2，所以是這樣：
...
Commit B-2     Commit B-2

Commit A-1     Commit A-1
...
4.我們在 new-feat 分支裡面，寫了新內容 3，提交了一個新的 commit 叫做 C，new-feat 這個分支會變成這樣：

master -            *new-feat -

Commit B-2     Commit C-3

Commit A-1     Commit B-2

​                          Commit A-1

5.現在我們 new-feat 的分支完成了，想要把它併回去 master 分支，所以先用 `git checkout master` 回到 master 分支，接著用 `git merge new-feat` 把 new-feat 併進去：

*master -          new-feat -

Commit C-3     Commit C-3

Commit B-2     Commit B-2

Commit A-1     Commit A-1

可以注意到 master 這個分支也有 Commit C-3 了。

6.最後，我們就可以把 new-feat 這個分支用 `git branch -d new-feat` 刪除，git 的狀態就會變成這樣：
...
然後我們各自在兩個 branch 新增了不同的內容：

*master -          new-feat -

Commit C-3     Commit C-4 

Commit B-2     Commit B-2

Commit A-1     Commit A-1

我們注意到，新版本 C 的內容一個是 3 而一個是 4，那當我們想進行 merge 的這個動作，git 就會告訴我們版本衝突了，並會顯示衝突的地方。

這時我們要**手動**去將衝突的地方修改，例如去把 commit C 的 3 改成 4，或是其他我們想要的內容，最後再用 `git commit -am` 提交一個新的版本出來，就把這個 conflict 解開了。
...
新開 branch > 發 Pull Requests > 工程師共同 review 無問題 > 合併 merger > 把原本的 branch 刪除